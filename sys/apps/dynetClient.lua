...
version="dynet prohlizec 1.0, modifikace pro OPUS Operacni System"
siteName=""
currentSite=""
siteID=-1
pageName=""
...
function refresh()
	...
	if webID==nil or webID==-1 then--find what went wrong
		    if webID==nil and ddns==false then drawError("Stranka nebyla nalezena (Spatne Napsano?)")
		elseif webID==nil and ddns==true then drawError("Zadna odpoved od DNS")
		elseif webID==-1 and ddns==true then drawError("Stranka nebyla nalezena na DNS (Spatne Napsano?)")
...
function interpret(text)
	...
	if command=="ask" then
		...
		print("Prosim Pockejte.....")--TODO place this nicely in the text entry field
		--note: server should send refresh as soon as it is done
	end
	--TODO add input command
end
function enterPage()
	siteName,pageName=split(popup("Adresa"),"/")
...
function renderMenu()
	...
	print("Tomas")
	print("")
	if ddnsID~=nil then print(" ID dedikovaneho DNS serveru: "..ddnsID) end
	if siteID~=nil then print(" ID stale stranky: "..siteID) end
	print(" Pouzit modem na strane: "..portSide)
	renderGUI()
end
...
if result==nil then
ddns=false 
print(" Varovani: DNS server nebyl nalezen!")
end
renderGUI()
enterPage()
renderGUI()
